...
#' @keywords internal
.pi_chord_type <- function(x, collapse = TRUE) {
  checkmate::qassert(x, "N+")
  x <- as.numeric(x)
  stopifnot(x[1] == 0,
            !collapse || (collapse && !anyDuplicated(x)),
...
#' @export
#' @rdname pi_chord_type
pi_chord_type.default <- function(x, collapse = TRUE) {
  pi_chord_type(pi_chord(x, collapse))
}
